...
public class BinarySearchTree<T extends Comparable<T>> implements BinarySearchTreeInterface<T> {
    ...
    public T getSmallestValue() {
        if(!isEmpty())
            return findNode(root,1).data; //recursive
        else
            return null; //no node in tree
    }
    
    public T getBiggestValue(){
        if(!isEmpty())
            return findNode(root,2).data; //recursive
        else
            return null; //no node in tree
    }
    
    private Node findNode(Node currentNode, int type){
        if(currentNode==null)
            return null;
        else if(type==1 && currentNode.left == null)
            return currentNode; //left-most node
        else if(type==1 && currentNode.right == null)
            return currentNode; //right-most node
        else if(type==1)
            return findNode(currentNode.left, 1);
        else if(type==2)
            return findNode(currentNode.right, 2);
        return null;
    }
// ReturnObject is the type for method removeEntry's 3rd parameter, oldEntry,
    ...
    private class PreorderIterator implements Iterator<T> {
        ...
        private void preorder(Node treeNode) {
            if (treeNode != null) {
                queue.enqueue(treeNode.data); //root
                preorder(treeNode.left); //left
                preorder(treeNode.right); //right
            }
        }
    ...
    private class PostorderIterator implements Iterator<T> {
        ...
        private void postorder(Node treeNode) {
            if (treeNode != null) {
                postorder(treeNode.left); //left
                postorder(treeNode.right); //right
                queue.enqueue(treeNode.data); //root
            }
        }
